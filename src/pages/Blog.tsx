  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-8 sm:space-y-10">
      ...
      <div className="text-center space-y-3">
        ...
        <h1 className="font-serif font-black text-2xl sm:text-4xl md:text-5xl text-ink dark:text-parchment tracking-tight">
          All Blog Posts
        </h1>
        <p className="max-w-sm sm:max-w-md mx-auto text-sm text-mist/80 font-sans leading-relaxed">
          Filter articles by tech tutorials, dev notes, and personal journal logs.
        </p>
      </div>

      {/* Category Filter Chips */}
      <div className="flex flex-wrap items-center justify-center gap-2 border-y border-mist/10 py-4 sm:py-6">
        {categories.map((cat) => (
          <button
            key={cat.id}
            onClick={() => setSelectedCategory(cat.id)}
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs font-mono font-semibold transition-all border outline-none focus:ring-2 focus:ring-brew-500 ${
      ...
      {filteredPosts.length === 0 ? (
        <div className="text-center py-14 sm:py-20 bg-cream/40 dark:bg-espresso/10 rounded-2xl border border-dashed border-mist/15 font-mono text-sm text-mist">
